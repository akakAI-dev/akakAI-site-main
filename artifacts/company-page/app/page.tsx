  ...
  return (
    <div className="fixed inset-[10px] rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex items-center gap-8 text-[14px] text-gray-400">
          {tabs.map((t) => (
            <button
              ...
              className={`transition-colors ${activeTab === t.id ? "text-gray-900" : "hover:text-gray-600"}`}
            >
              {t.label}
            </button>
          ))}
          <span className="text-gray-200">|</span>
          <a href="#" className="hover:text-gray-600 transition-colors">Sign In</a>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto page-scroll">
        ...
        {/* HERO */}
        <div className="px-8 pt-14 pb-12 border-b border-gray-100">
          <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">
            {activeTab === "about" ? "Company Overview" : activeTab === "mission" ? "Our Mission" : "The Team"}
          </p>
          <h1 className="text-[64px] leading-[1.0] tracking-tight text-gray-900 mb-7" style={{ fontWeight: 500 }}>
            {activeTab === "about" && <>AI that acts,<br /><span className="text-gray-200">not just reacts.</span></>}
            {activeTab === "mission" && <>Action is the<br /><span className="text-gray-200">default.</span></>}
            {activeTab === "team" && <>The people<br /><span className="text-gray-200">building it.</span></>}
          </h1>
          <p className="text-[19px] text-gray-400 leading-[1.7] max-w-2xl" style={{ fontWeight: 400 }}>
        ...
        {activeTab === "about" && (
          <div className="px-8 py-14 space-y-16">
            ...
            {/* What we are */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">What We Are</p>
              <div className="grid grid-cols-2 gap-12">
                <div>
                  <h2 className="text-[32px] leading-tight text-gray-900 mb-6" style={{ fontWeight: 500 }}>
                    A new class of intelligent systems.
                  </h2>
                  <p className="text-[18px] text-gray-500 leading-[1.8]">
                ...
                <div className="pt-2">
                  <p className="text-[18px] text-gray-500 leading-[1.8] mb-6">
            ...
            <div className="border-l-2 border-gray-200 pl-8 py-2">
              ...
              <p className="text-[13px] uppercase tracking-[0.15em] text-gray-300 mt-5">The core belief driving everything we build</p>
            </div>

            {/* What our agents do */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">What Our Agents Do</p>
              <div className="grid grid-cols-2 gap-5">
                {[
                  { label: "Independent Thinking", desc: "Real reasoning — not pattern-matching. Our agents decompose goals, weigh options, and find the right path even when the instructions stop short of telling them how." },
                  { label: "Initiative", desc: "They move first. When there's work to be done, the agent doesn't wait for a signal. It identifies what's needed, builds a plan, and starts — without asking for permission." },
                  { label: "Goal Execution", desc: "The distance between a stated objective and a real-world outcome is where most AI falls apart. Our agents bridge it — end-to-end, from the first step to the final delivery." },
                  { label: "Cross-System Coordination", desc: "The work of the world spans dozens of tools, APIs, and platforms. Our agents coordinate across every system they need to, simultaneously, without losing the thread." },
                  { label: "Judgment Under Uncertainty", desc: "Plans break. Conditions change. The real test of an intelligent system is what it does when the world doesn't cooperate — and ours make the right call without escalating." },
                  { label: "Natural Scaling", desc: "More complexity doesn't mean more humans. Our agents compound in capability as scope grows — handling more, deciding more, without the overhead that comes with scaling teams." },
                ].map((c, i) => (
                  <div key={i} className="bg-gray-50 rounded-xl p-7">
                    <p className="text-[17px] text-gray-900 mb-4" style={{ fontWeight: 500 }}>{c.label}</p>
                    <p className="text-[16px] text-gray-400 leading-[1.75]">{c.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* The technology */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">The Technology</p>
              <div className="grid grid-cols-2 gap-12">
                <div>
            ...
            {/* Beliefs */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">What We Believe</p>
              <div className="flex flex-wrap gap-3">
                {["AI shouldn't wait for direction", "Move first, iterate faster", "Capability over complexity", "Outcomes, not just outputs", "Strip the gimmicks", "Action is the default"].map((b) => (
                  <span key={b} className="text-[15px] text-gray-500 border border-gray-200 px-5 py-2.5 rounded-full">
                    {b}
                  </span>
                ))}
              </div>
        ...
        {activeTab === "mission" && (
          <div className="px-8 py-14 space-y-16">
            ...
            <div className="grid grid-cols-2 gap-12">
              <div>
                <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">The Mission</p>
              ...
              <div className="grid grid-cols-2 gap-5">
                {[
                  ...
                  <div key={i} className="bg-gray-50 rounded-xl p-8">
                    ...
                    <p className="text-[16px] text-gray-400 leading-[1.8]">{b.body}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Four principles */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">Four Principles</p>
              <div className="grid grid-cols-2 gap-5">
                {[
                  ...
                  { word: "Adapt", desc: "The world doesn't hold still. Conditions change, systems fail, goals shift. Agents that can only follow scripts break the moment reality deviates from the plan. Ours don't." },
                  { word: "Act", desc: "Thinking without doing is just noise. Our agents close the loop — from high-level intent to real-world outcome, without waiting for permission, clarification, or a next step." },
                  { word: "Scale", desc: "Human attention is finite. Intelligent action shouldn't be. As complexity grows, the agents grow with it — compounding capability without compounding cost or headcount." },
                ].map((p, i) => (
                  <div key={i} className="border border-gray-100 rounded-xl p-8">
                    ...
                    <p className="text-[16px] text-gray-400 leading-[1.8]">{p.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* The next wave */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">The Next Wave</p>
              <div className="grid grid-cols-2 gap-12">
                <div>
                  <h2 className="text-[28px] leading-tight text-gray-900 mb-6" style={{ fontWeight: 500 }}>
            ...
            {/* What we are not */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">What akakAI Is Not</p>
              <div className="grid grid-cols-2 gap-5">
                {[
                  { label: "A chatbot", desc: "Chatbots respond. Agents act. The architecture, the intent, and the outcome are entirely different." },
                  { label: "A workflow tool", desc: "We don't need pre-built workflows. The agent figures out the steps, adapts when they fail, and finds a way regardless." },
                  { label: "An AI wrapper", desc: "Purpose-built for autonomous execution from the ground up — not a UI layer or another chatbot with a new coat of paint." },
                  { label: "A hype product", desc: "No buzzwords, no fundraising theater. Just agents that work — reliably, independently, in the real world with real stakes." },
                ].map((item, i) => (
                  <div key={i} className="border border-gray-100 rounded-xl p-8">
                    <p className="text-[18px] text-gray-300 line-through mb-4" style={{ fontWeight: 400 }}>{item.label}</p>
                    <p className="text-[16px] text-gray-400 leading-[1.8]">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === "team" && (
          <div className="px-8 py-14 space-y-16">
            ...
            <div className="grid grid-cols-2 gap-12">
              <div>
                <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">Founder</p>
                <h2 className="text-[42px] leading-none text-gray-900 mb-3" style={{ fontWeight: 500 }}>Zayd Malik</h2>
                <p className="text-[14px] text-gray-300 mb-8 uppercase tracking-[0.12em]">akakAI</p>
                <p className="text-[22px] text-gray-900 leading-[1.4] mb-8" style={{ fontWeight: 400 }}>
                  "AI should act, not just react."
                </p>
                <p className="text-[13px] uppercase tracking-[0.15em] text-gray-300">The conviction that started it all</p>
              </div>
              <div className="pt-14">
                <p className="text-[18px] text-gray-500 leading-[1.8] mb-6">
            ...
            <div className="grid grid-cols-2 gap-12">
              <div>
                <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">The Approach</p>
                <h2 className="text-[28px] leading-tight text-gray-900 mb-6" style={{ fontWeight: 500 }}>
            ...
            {/* What drives him */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-8">What Drives This</p>
              <div className="grid grid-cols-2 gap-5">
                {[
                  { title: "The Problem", text: "Passive tools, overhyped AI, systems that wait for input instead of taking initiative. The gap between what AI promised and what it delivered." },
                  { title: "The Vision", text: "Agents that take initiative, understand context, and follow through from start to finish. AI that makes action the default, not the exception." },
                  { title: "The Standard", text: "No trends, no noise. Just AI that works reliably — without getting in the way, without needing a prompt for every decision." },
                  { title: "The Goal", text: "Close the gap between what you want done and what gets done. Permanently. Without the intermediary layer that slows everything down." },
                ].map((item, i) => (
                  <div key={i} className="bg-gray-50 rounded-xl p-7">
                    <p className="text-[16px] text-gray-900 mb-4" style={{ fontWeight: 500 }}>{item.title}</p>
                    <p className="text-[16px] text-gray-400 leading-[1.8]">{item.text}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Tags */}
            <div>
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-6">Zayd in Tags</p>
              <div className="flex flex-wrap gap-3">
                {["Anti-hype", "Action-first", "Systems thinker", "Capability over complexity", "Builder", "No gimmicks"].map((tag) => (
                  <span key={tag} className="text-[14px] text-gray-400 border border-gray-200 px-5 py-2.5 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="border border-gray-100 rounded-xl p-10">
              <p className="text-[12px] uppercase tracking-[0.18em] text-gray-300 mb-6">We're Hiring</p>
              <h3 className="text-[28px] text-gray-900 leading-tight mb-5" style={{ fontWeight: 500 }}>
                Build what comes after passive AI.
              </h3>
              <p className="text-[18px] text-gray-400 leading-[1.75] mb-8 max-w-lg">
        ...
        <div className="border-t border-gray-100 px-8 py-6 flex items-center justify-between">
          ...
          />
          <div className="flex gap-7 text-[13px] text-gray-300">
